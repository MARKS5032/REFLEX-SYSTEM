...
if (!$id) {
    ...
    header('Location: /reflex/retailer/deliveries.php');
    exit;
}
...
// RULE 1: a retailer can only view deliveries they created.
if (!$delivery || (int) $delivery['retailer_id'] !== (int) $user['id']) {
    flash('error', 'That delivery was not found, or does not belong to your account.');
    header('Location: /reflex/retailer/deliveries.php');
    exit;
}
...
<div class="page-header">
  <div>
    ...
    <p>Created <?= date('d M Y, H:i', strtotime($delivery['created_at'])) ?></p>
  </div>
  <a href="/reflex/retailer/deliveries.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Back</a>
</div>
      ...
      <div class="reflex-card-body">
        ...
        <?php else: ?>
          <div class="empty-state py-3">
            ...
            <h5 style="font-size:0.95rem;">No rider assigned yet</h5>
            <p class="mb-0 small">A dispatcher will assign a rider shortly.</p>
          </div>
        <?php endif; ?>
      </div>
    </div>
